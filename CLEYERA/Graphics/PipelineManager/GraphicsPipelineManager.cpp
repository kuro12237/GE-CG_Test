...
void GraphicsPipelineManager::CreatePSO(SPSO &pso)
{
	...
	pso.PhongModel = CreatePhong(device.Get(), commands, shader.Phong_Model);
	LogManager::CompliteLog("PhongModelPSO");
	pso.TestNormalModel = CreateTestNormalModel(device.Get(), commands, shader.TestNormalModel);
	LogManager::CompliteLog("TestNormalModelPSO");
}
...
SPSOProperty GraphicsPipelineManager::CreateTestNormalModel(ComPtr<ID3D12Device> device, Commands commands, SShaderMode shader)
{
